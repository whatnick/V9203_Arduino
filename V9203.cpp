...
*/
    ...
    for( ;len > 0; len--)
    {
        ucData = (unsigned char) (crc / 0x100);    //暂存crc高8位
        crc <<= 8;
...
void JbpmInit(void)
{
    ...
    //BE_ReadP(EEP_JBTOTAL,(unsigned char*)&gs_JbPm,sizeof(S_JBPM));
	/*
    if(gs_JbPm.ui_JbCRC == do_CRC((unsigned char *)&gs_JbPm,sizeof(S_JBPM)-2))      //RAM中的数据是否完整
    {                                                                       //因为以下参数要求定义在两个结构体里面
        EnyB_JbPm_Updata();
    }
    else
...
//        gs_JbPm.gs_JBC.RacREWWAPT = 0x00000000;  //全波无功功功率二次补偿寄存器   
        ...
        gs_JbPm.ui_JbCRC=do_CRC((unsigned char*)&gs_JbPm,sizeof(S_JBPM)-2);     // 校表参数的CRC结果
